...
class AdvancedLocalModelManager:
    ...
    async def download_model(
        self, model_name: str, progress_callback=None
    ) -> bool:
        """
        ...
        try:
            ...
            snapshot_download(
                repo_id=model_config.url.split("/")[-2]
                + "/"
                + model_config.url.split("/")[-1],
    ...
    def load_model(self, model_name: str, device: str = "auto") -> Any:
        ...
        if not model_config.is_downloaded:
            raise RuntimeError(
                f"Model '{model_name}' not downloaded. Run download_model() first."
            )
    ...
    def get_best_model_for_task(
        self, task: str, constraints: Dict[str, Any] = None
    ) -> str:
        """
            ...
            if task in config.capabilities:
                ...
                if constraints:
                    if "max_memory" in constraints:
                        memory_gb = int(
                            config.memory_requirement.replace("GB", "")
                        )
    ...
    def _save_model_status(self) -> None:
        ...
        status = {
            name: config.is_downloaded for name, config in self.models.items()
        }

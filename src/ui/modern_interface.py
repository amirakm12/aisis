...
class ModernThemeManager:
    ...
    def apply_theme(self, app: QApplication, theme: Theme) -> None:
        ...
        # Set colors
        palette.setColor(
            QPalette.ColorRole.Window, QColor(color_scheme.background)
        )
        palette.setColor(
            QPalette.ColorRole.WindowText, QColor(color_scheme.text)
        )
        palette.setColor(QPalette.ColorRole.Base, QColor(color_scheme.surface))
        palette.setColor(
            QPalette.ColorRole.AlternateBase, QColor(color_scheme.background)
        )
        palette.setColor(
            QPalette.ColorRole.ToolTipBase, QColor(color_scheme.surface)
        )
        palette.setColor(
            QPalette.ColorRole.ToolTipText, QColor(color_scheme.text)
        )
        palette.setColor(QPalette.ColorRole.Text, QColor(color_scheme.text))
        palette.setColor(
            QPalette.ColorRole.Button, QColor(color_scheme.surface)
        )
        palette.setColor(
            QPalette.ColorRole.ButtonText, QColor(color_scheme.text)
        )
        palette.setColor(
            QPalette.ColorRole.BrightText, QColor(color_scheme.accent)
        )
        palette.setColor(QPalette.ColorRole.Link, QColor(color_scheme.primary))
        palette.setColor(
            QPalette.ColorRole.Highlight, QColor(color_scheme.primary)
        )
        palette.setColor(
            QPalette.ColorRole.HighlightedText, QColor(color_scheme.text)
        )
...
class ModernMainWindow(QMainWindow):
    ...
    def connect_sidebar_signals(self):
        ...
        for i, btn in enumerate(self.sidebar.nav_buttons):
            btn.clicked.connect(
                lambda checked, index=i: self.content_stack.setCurrentIndex(
                    index
                )
            )
    ...
    def toggle_theme(self):
        ...
        btn_text = (
            "🌙 Dark Mode" if new_theme == Theme.LIGHT else "☀️ Light Mode"
        )

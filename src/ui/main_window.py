...
class SolutionsDialog(QDialog):
    ...
    def __init__(self, solutions, critiques, parent=None):
        ...
        for i, sol in enumerate(solutions):
            text += (
                f"Solution {i+1} (Agent: {sol['agent']}):\n{sol['result']}\n\n"
            )
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.orchestrator.register_agent(
                "text_recovery", TextRecoveryAgent()
            )
            self.orchestrator.register_agent(
                "meta_correction", MetaCorrectionAgent()
            )
            self.orchestrator.register_agent(
                "semantic_editing", SemanticEditingAgent()
            )
            self.orchestrator.register_agent(
                "auto_retouch", AutoRetouchAgent()
            )
            self.orchestrator.register_agent("generative", GenerativeAgent())
            self.orchestrator.register_agent(
                "neural_radiance", NeuralRadianceAgent()
            )
            self.orchestrator.register_agent(
                "super_resolution", SuperResolutionAgent()
            )
            self.orchestrator.register_agent(
                "color_correction", ColorCorrectionAgent()
            )
            self.orchestrator.register_agent(
                "tile_stitching", TileStitchingAgent()
            )
            self.orchestrator.register_agent(
                "feedback_loop", FeedbackLoopAgent()
            )
            self.orchestrator.register_agent(
                "perspective_correction", PerspectiveCorrectionAgent()
            )
            self.orchestrator.register_agent(
                "material_recognition", MaterialRecognitionAgent()
            )
            self.orchestrator.register_agent(
                "damage_classifier", DamageClassifierAgent()
            )
            self.orchestrator.register_agent(
                "hyperspectral_recovery", HyperspectralRecoveryAgent()
            )
            self.orchestrator.register_agent(
                "paint_layer_decomposition", PaintLayerDecompositionAgent()
            )
            self.orchestrator.register_agent(
            ...
            self.orchestrator.register_agent(
                "context_aware_restoration", ContextAwareRestorationAgent()
            )
            self.orchestrator.register_agent(
                "adaptive_enhancement", AdaptiveEnhancementAgent()
            )
            self.orchestrator.register_agent(
        ...
        if not torch.cuda.is_available():
            self.status_bar.showMessage(
                "Warning: CUDA GPU not detected. Running in CPU mode."
            )
    ...
    def _setup_ui(self):
        ...
        self.tot_button.setToolTip(
            "Try multiple agents, critique, and pick best"
        )
    ...
    def open_image(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Open Image", str(Path.home()), "Images (*.png *.jpg *.jpeg)"
        )
    ...
    def _voice_loop(self):
        ...
        voice_manager.start_voice_loop(
    ...
    def _append_chat(self, message, user=False, thinking=False):
        ...
        prefix = "🧑 " if user else "🤖 "
        icon = "🟢" if user else ("💬" if not thinking else "⏳")
        item = QListWidgetItem(f"{icon} {prefix}[{timestamp}] {message}")
        if thinking:
    ...
    def _update_live_transcript(self, partial):
        ...
        if partial:
            if self.chat_panel.count() and self.chat_panel.item(
                self.chat_panel.count() - 1
            ).text().startswith("🧑 ["):
                self.chat_panel.item(self.chat_panel.count() - 1).setText(
                    f"🧑 [Live] {partial}"
                )
            else:
    ...
    def _parse_intent(self, text):
        ...
        if any(
            ...
            word in text_l for word in ["again", "more", "repeat", "another"]
        ):
            return "followup", {}
        elif any(
            word in text_l
            for word in ["hello", "hi", "how are you", "what can you do"]
        ):
            return "chat", {}
        else:
            return "unknown", {}
    ...
    def _run_image_edit(self, instruction):
        async def edit_and_update():
            ...
            if result and result.get("output_image"):
                ...
                self._append_chat(
                    "AISIS: Edit failed or not recognized.", user=False
                )
    ...
    def export_chat(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Chat", "chat.txt", "Text Files (*.txt)"
        )
        if file_path:
    ...
    def _on_sketch_made(self, qimage):
        ...
        self._append_chat(
            "AISIS: Sketch received. Processing...", user=False, thinking=True
        )
    ...
    def _handle_user_intent(self, text, input_mode="text"):
        ...
        if intent in [
            ...
            if self.current_image or input_mode in ("sketch", "gesture"):
                ...
                self._append_chat(
                    "AISIS: Processing your request...",
                    user=False,
                    thinking=True,
                )
                self.start_operation(
                    lambda: self._process_orchestrator_task(task)
                )
            else:
                self._append_chat(
                    "AISIS: Please load an image first.", user=False
                )
        elif intent == "chat":
            ...
            self._append_chat(
                "AISIS: Explaining...", user=False, thinking=True
            )
            ...
            self._append_chat(
                "AISIS: Sorry, I didn't understand. Please try a different command.",
                user=False,
            )
    ...
    def _process_orchestrator_task(self, task):
        ...
        if result and isinstance(result, dict):
            ...
            if "meta_critique" in result:
                self._append_chat(
                    f"Meta-agent critique: {result['meta_critique']}",
                    user=False,
                )
            if "agent_reports" in result:
                for agent, report in result["agent_reports"].items():
                    self._append_chat(f"{agent}: {report}", user=False)
        else:
            self._append_chat(
    ...
    def run_tree_of_thought(self):
        ...
        if not self.current_image:
            ...
            return
        last_instruction = (
            self._get_last_edit_instruction() or "restore this image"
        )
        ...
        if not agent_names:
            self._append_chat(
                "AISIS: No agents registered for ToT.", user=False
            )
            return
        self._append_chat(
            "AISIS: Running Tree-of-Thought reasoning...",
            user=False,
            thinking=True,
        )
        ...
        def on_done(result):
            self._replace_last_chat(
                "AISIS: Tree-of-Thought complete.", user=False
            )
            dlg = SolutionsDialog(
                result["all_solutions"], result["critiques"], self
            )
            dlg.exec()
            # Optionally, use dlg.get_feedback() for further learning

        worker = threading.Thread(
            target=lambda: on_done(do_tot()), daemon=True
        )
        worker.start()

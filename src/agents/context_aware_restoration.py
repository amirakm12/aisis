...
class ContextAwareRestorationAgent(BaseAgent):
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            self.models["context_classifier"] = (
                await self._load_context_classifier()
            )

            # Content-aware restoration network
            self.models["content_restorer"] = (
                await self._load_content_restorer()
            )

            # Semantic segmentation network
            self.models["semantic_segmenter"] = (
                await self._load_semantic_segmenter()
            )

            # Adaptive restoration network
            self.models["adaptive_restorer"] = (
                await self._load_adaptive_restorer()
            )

            # Context-specific enhancement network
            self.models["context_enhancer"] = (
                await self._load_context_enhancer()
            )
            ...
            logger.error(
                f"Failed to initialize context-aware restoration models: {e}"
            )
            raise
    ...
    async def _process(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            adaptive_restored = await self._apply_adaptive_restoration(
                x, context_probs
            )
    ...
    def _analyze_context(
        ...
        analysis = {
            ...
            "context_distribution": dict(
                zip(self.context_types, probs.tolist())
            ),
            "secondary_contexts": [],
        }

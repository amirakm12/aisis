...
class AutoRetouchAgent(BaseAgent):
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            self.models["face_detection"] = (
                await self._load_face_detection_model()
            )

            # Face enhancement model
            self.models["face_enhancement"] = (
                await self._load_face_enhancement_model()
            )

            # Body detection model
            self.models["body_detection"] = (
                await self._load_body_detection_model()
            )
    ...
    async def _apply_enhancements(
        self, x: torch.Tensor, face_mask: torch.Tensor, body_mask: torch.Tensor
    ) -> torch.Tensor:

...
class GenerativeAgent(BaseAgent):
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            self.models["text_to_image"] = (
                await self._load_text_to_image_model()
            )

            # Image-to-image model
            self.models["image_to_image"] = (
                await self._load_image_to_image_model()
            )
    ...
    async def _load_text_to_image_model(self) -> nn.Module:
        ...
        class DummyDiffusionModel(nn.Module):
            ...
            def forward(
                self, prompt, height=512, width=512, num_inference_steps=1
            ):
    ...
    async def _text_to_image(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with torch.no_grad():
            generated = self.models["text_to_image"](
